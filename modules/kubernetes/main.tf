...
clusters:
- cluster:
    server: ${yandex_kubernetes_cluster.kube_cluster.master[0].internal_v4_endpoint}
    certificate-authority-data: ${base64encode(yandex_kubernetes_cluster.kube_cluster.master[0].cluster_ca_certificate)}
  name: kubernetes
contexts:
- context:
    cluster: kubernetes
